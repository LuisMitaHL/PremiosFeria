...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object('success', false, 'reason', 'No registrado');
  END IF;
  ...
  IF v_participant.points < v_reward.cost THEN
    RETURN jsonb_build_object('success', false, 'reason',
      'Necesitas ' || (v_reward.cost - v_participant.points)::TEXT || ' puntos más');
  END IF;

  IF v_reward.stock <= 0 THEN
    RETURN jsonb_build_object('success', false, 'reason',
      'Agotado: ya no quedan unidades de este premio');
  END IF;

  -- Los pre-chequeos de arriba solo dan mensajes amigables; la puerta real son
  -- los UPDATE condicionales: la condición vive en el WHERE, así que dos
  ...
  -- El bloque con EXCEPTION es una subtransacción: si algo falla dentro, se
  -- revierte TODO lo escrito en él, así nunca queda stock descontado sin
  -- puntos debitados ni al revés.
  BEGIN
    ...
    VALUES (v_participant.id, p_reward_id);
  EXCEPTION
    WHEN unique_violation THEN
      RETURN jsonb_build_object('success', false, 'reason', 'Ya reclamaste este premio');
    WHEN OTHERS THEN
